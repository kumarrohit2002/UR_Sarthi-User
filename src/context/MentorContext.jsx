...
export default function MentorContextProvider({ children }) {
  ...
  const getAllMentorData = async () => {
    ...
    try {
      const response = await axios.post(
        `${baseUrl}/api/v1/mentor/getAllMentorProfiles`
      );
  ...
  const getMentorAboutData = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${baseUrl}/api/v1/mentor/getprofile`, {
        Id,
      });
  ...
  const bookAppointment = async (
    ...
    try {
      const response = await axios.post(
        `${baseUrl}/api/v1/appointment/book-appointment`,
        {
  ...
  const getMyBooking = async () => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/api/v1/appointment/getmy-booking`,
        {
  ...
  const searchHandler = async (searchValue) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/api/v1/mentor/searchMentor`,
  ...
  const getAllJobPortal = async () => {
    ...
    try {
      const response = await axios.post(
        `${baseUrl}/api/v1/mentor/get-all-job-posts`,
        {}
      );
  ...
  const CheckoutHandler = async (
    ...
    try {
      ...
      const {
        data: { order },
      } = await axios.post(`${baseUrl}/api/v1/payment/payment-checkout`, {
        name,
        amount,
      });
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            ...
            await bookAppointment(mentorAboutData, selectedDate, selectedTime,response);
...
// {
//     "razorpay_payment_id": "pay_QvJJh3Dn2wcw8v",
//     "razorpay_order_id": "order_QvJI5bUDBpEblk",
//     "razorpay_signature": "08373d381006314bb65ede58a22fe53c0e4dfda6afd588e6e48958abe91c303b"
// }